...
class DeepQNetwork:
    ...
    def _build_net(self):
        ...
        def build_layers(s, c_names, n_l1, w_initializer, b_initializer):
            ...
            # s = tf.reshape(s, [-1, self.n_width, self.n_height, self.n_channel])
            n_filter = 32
            ...
            #     conv4 = tf.layers.average_pooling2d(s, (3, 3), 1, padding='SAME')
            ...
            # with tf.variable_scope('concat') as scope:
                ...
                # bias = tf.get_variable(name='biases', initializer=b_initializer, shape=[n_filter * 4], collections=c_names)
                # inception1 = tf.nn.relu(tf.nn.bias_add(inception1, bias))
                # fc = tf.layers.average_pooling2d(inception1, (8, 8), (8, 8), padding='SAME')
                # fc = tf.contrib.layers.flatten(fc)
                # fc = tf.reshape(fc, (tf.shape(fc)[0], self.n_width, n_filter * 2))
                # fc = tf.contrib.layers.flatten(s)
            ...
            with tf.variable_scope('l1'):
                w1 = tf.get_variable('w1', [self.n_height, n_l1], initializer=w_initializer, collections=c_names)
                b1 = tf.get_variable('b1', [1, n_l1], initializer=b_initializer, collections=c_names)
                fc = tf.nn.relu(tf.matmul(s, w1) + b1)
        ...
        with self.graph.as_default() as graph:
            ...
            self.s = tf.placeholder(tf.float32, [None, self.n_height], name='s')  # input
            ...
            self.s_ = tf.placeholder(tf.float32, [None, self.n_height], name='s_')    # input
            ...
            with tf.variable_scope('summary') as scope:
                scalar_summary_tags = ['loss_avg', 'q_max', 'q_total', 'epsilon']
